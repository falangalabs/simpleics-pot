...
def main() -> int:
    ...
    try:
        ...
        initial = client.read_holding_registers(0, count=7, device_id=1)
        if initial.isError():
            raise RuntimeError(f"initial read failed: {initial}")
        write = client.write_register(5, 8000, device_id=1)
        if write.isError():
            raise RuntimeError(f"bounded setpoint write failed: {write}")
        final = client.read_holding_registers(5, count=1, device_id=1)
        if final.isError():
