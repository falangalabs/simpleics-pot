...
class ProcessModelTests(unittest.TestCase):
    ...
    def test_auto_mode_starts_only_above_upper_hysteresis(self) -> None:
        minimum_setpoint = int(
            self.model.register_map.require_key("level_setpoint").minimum
        )
        self.model.write_raw("level_setpoint", minimum_setpoint)
        for _ in range(25):
            self.model.tick()
        self.assertTrue(self.model.snapshot_raw()["pump_feedback"])
    ...
    def test_valve_feedback_is_delayed(self) -> None:
        ...
        elapsed = 0.0
        step = min(0.5, self.model.actuator_delay_seconds / 2)
        while elapsed + step < self.model.actuator_delay_seconds:
            self.model.tick(step)
            elapsed += step
            self.assertTrue(self.model.snapshot_raw()["inlet_valve_feedback"])
        self.model.tick(self.model.actuator_delay_seconds - elapsed)
